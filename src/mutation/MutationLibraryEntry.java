...
/**
 * An entry in the {@link MutationLibrary} . Warps a {@link Mutation}
 * 
 * @see MutationLibrary
 */
public class MutationLibraryEntry {
	...
	/**
	 * List of vectors representing the offset position from the first monomer
	 * of the mutation. it will be used to determine if the mutation can be
	 * applied on the protein with no collisions
	 **/
	 ...
	 *            the degree
	 * @param direction
	 *            the direction of a monomer
	 * @param dimensions
	 *            the system dimensionality.
	 */
	public MutationLibraryEntry(Mutation mutation, char axis, double degree,
			MonomerDirection direction, Dimensions dimensions,
			MatrixManipulation matrixman) {
		...
		for (int i = 0; i < conf.length - 1; i++) {
			...
			positionOffsets.add(matrixman.LorentzTransformation(vecs[0], axis,
					degree));
		}
	}
	...
	public String toString() {
		return mutation.toString() + "\taxis:" + getAxis() + "\tdegree:"
				+ getDegree() + "\tDirection:" + getDirection();
	}
